...
$runnerLogFile = Join-Path $runnerLog 'runner-scout.log'
...
$tmpOut = Join-Path $env:TEMP ("naukri-scout-" + [guid]::NewGuid().ToString('N') + '.log')
& $nodeBin (Join-Path $modDir 'naukri-scout.js') @args > $tmpOut 2>&1
$code = $LASTEXITCODE
if (Test-Path -LiteralPath $tmpOut) {
    $null = Get-Content -LiteralPath $tmpOut | Tee-Object -FilePath $runnerLogFile -Append
    Remove-Item -LiteralPath $tmpOut -Force -ErrorAction SilentlyContinue
}
Write-Log "Scout bot exit code: $code"
exit $code